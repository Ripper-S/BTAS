  ...
  searchEngines.forEach(engine => {
    GM_registerMenuCommand(engine.name, () => {
      ...
      const searchURL = engine.url
        ...
        .replace('{cur_logsourceDomain}', cur_logsourceDomain);
...
function editNotify() {
  ...
  function addEditonClick() {
    ...
    if (
      DecoderName.includes('esf') ||
      DecoderName.includes('swireproperties') ||
      DecoderName.includes('lsh-hk')
    ) {
      $('#edit-issue').on('click', () => {
    ...
    for (const info of alertInfo) {
      ...
      if (orgNavigator) {
        ...
        showFlag(
          'error',
          '',
          `There is no <strong>${DecoderName}</strong> Navigator on Cortex`,
          'auto'
        );
      }
    }
